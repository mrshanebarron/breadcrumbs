        ...
        @foreach($this->items as $index => $item)
            <li style="display: flex; align-items: center;">
                @if($index > 0)
                    ...
                    @elseif($this->separator === 'slash')
                        <span style="color: #d1d5db; margin: 0 0.75rem; font-weight: 300;">/</span>
                    @elseif($this->separator === 'dot')
                        <span style="color: #9ca3af; margin: 0 0.75rem; font-size: 0.5rem;">&bull;</span>
                    @else
                        <span style="color: #9ca3af; margin: 0 0.75rem;">{{ $this->separator }}</span>
                    @endif
                @endif

                @if(isset($item['href']) && $index < count($this->items) - 1)
